...
function get_data_array() {
    var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            ...
            pivot_array = data_array[0];
            data_array = data_array.splice(1,data_array.length-1);
				        ...
				        xhttp2.onreadystatechange = function() {
				            ...
				            for (var j = ground_data_array.length - 1; j >= 0; j--) {
				            	...
				            	if(j == 0){
								    ...
								    xhttp2.open("GET", "https://sheets.googleapis.com/v4/spreadsheets/1PS8Cbm_HYHBDo9e0fy4p8bEKmd39g0aACBlWvczW2F0/values/Sheet4?key=AIzaSyAmcp44cOi9-6XM4EqjCjIQLbj_D__1YPE");
								    xhttp2.send();
				            	}
				            }
				        };
	...
	function create_chs(data_array){
		...
		for (var i = 26; i > 0; i--) {
				...
				materials_obj[i] = Array(6)
				faceMaterial_obj[i] = Array(6)
				...
				faceMaterial_obj[i][3] = [
				    new THREE.MeshBasicMaterial( {color: 0xe2e2e2} ), //side
				    new THREE.MeshBasicMaterial( {color: 0xe2e2e2} ),
				    new THREE.MeshBasicMaterial( {color: 0xe2e2e2} ),
				    new THREE.MeshBasicMaterial( {color: 0xe2e2e2} ),
				    new THREE.MeshBasicMaterial( { map : texture0_obj[i][0],
										          color: 0xe2e2e2} ),
				    new THREE.MeshBasicMaterial( {color: 0xe2e2e2} ) //front
				];
				faceMaterial_obj[i][4] = [
				    new THREE.MeshBasicMaterial( {color: 0xc1c1c1} ), //side
				    new THREE.MeshBasicMaterial( {color: 0xc1c1c1} ),
				    new THREE.MeshBasicMaterial( {color: 0xc1c1c1} ),
				    new THREE.MeshBasicMaterial( {color: 0xc1c1c1} ),
				    new THREE.MeshBasicMaterial( { map : texture0_obj[i][0],
										          color: 0xc1c1c1} ),
				    new THREE.MeshBasicMaterial( {color: 0xc1c1c1} ) //front
				];
				faceMaterial_obj[i][5] = [
				    new THREE.MeshBasicMaterial( {color: 0xa5a5a5} ), //side
				    new THREE.MeshBasicMaterial( {color: 0xa5a5a5} ),
				    new THREE.MeshBasicMaterial( {color: 0xa5a5a5} ),
				    new THREE.MeshBasicMaterial( {color: 0xa5a5a5} ),
				    new THREE.MeshBasicMaterial( { map : texture0_obj[i][0],
										          color: 0xa5a5a5} ),
				    new THREE.MeshBasicMaterial( {color: 0xa5a5a5} ) //front
				];
		}
		faceMaterial_obj[27] = [
		    new THREE.MeshBasicMaterial( {color: 0x2453ff} ), //side
	...
	function get_obj_position(j){
		...
		if(j<3){ pivot_wrapper_sky[j] = new THREE.Group(); }
		mainpivot.add( pivot_wrapper[j] );
		mainpivot.add( pivot_wrapper_cube[j] );
		if(j<3){ mainpivot.add( pivot_wrapper_sky[j] ); }
		...
		if(j<3){ pivot_wrapper_sky[j].position.set(0, -0.5*getheight(unit_obj*Math.abs( pivot_wrapper.length/2 - j ),unit_obj*radius), unit_obj*j-unit_obj*data_array[0].length/2) }
        ...
        for (var k = data_array.length-1; k >= 0; k--) {
			...
			if(k<30 && j < 3){
				...
				if(sky_data_array[k][j] === '_'){}else{
					var sel_sky = new THREE.Mesh( geometry,faceMaterial_obj[ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])][2]);
					sel_sky.position.set(0, unit_obj*(radius-10), 0);
					pivot_single_sky[k].add(sel_sky);
						 if(k<10 && j==0){
						 	pivot_single_sky[k].rotation.z = degtorad(2.25)*k
						 	sky_initpos_array.push(degtorad(2.25)*k)
						 }
					else if(k<10 && j==1){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(10)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(10))
					}
					else if(k<10 && j==2){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(20)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(20))
					}
					else if(k<20 && j==0){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(120)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(120))
					}
					else if(k<20 && j==1){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(120) + degtorad(10)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(120) + degtorad(10))
					}
					else if(k<20 && j==2){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(120) + degtorad(20)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(120) + degtorad(20))
					}
					else if(k<30 && j==0){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(240)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(240))
					}
					else if(k<30 && j==1){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(240) + degtorad(10)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(240) + degtorad(10))
					}
					else if(k<30 && j==2){
						pivot_single_sky[k].rotation.z = degtorad(2.25)*k + degtorad(240) + degtorad(20)
						sky_initpos_array.push(degtorad(2.25)*k + degtorad(240) + degtorad(20))
					}
			...
			if(data_array[k][j] === '_'){}else{
				if(data_array[k][j] === '*'||data_array[k][j] === '.'){
						...
						if(data_array[k][j].split('/').length > 1){
							link = data_array[k][j]
							title = data_array[k][j].split('en/')[1].replaceAll('-', ' ')
							group_array[parseInt(Math.floor(k/8)*3)+parseInt(Math.floor(j/17))][0].push(link)
							group_array[parseInt(Math.floor(k/8)*3)+parseInt(Math.floor(j/17))][0].push(title)
						}else {
								if(data_array[k][j].split('.').length == 3){
									...
									// sel_cube.name = ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])
									...
									if(floor < 2){
									var sel_cube = new THREE.Mesh( geometry,faceMaterial_obj[ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])][5]);
									// sel_cube.name = ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])
									}
									else if(floor < 4){
									var sel_cube = new THREE.Mesh( geometry,faceMaterial_obj[ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])][4]);
									// sel_cube.name = ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])
									}
									else if(floor < 6){
									var sel_cube = new THREE.Mesh( geometry,faceMaterial_obj[ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])][3]);
									// sel_cube.name = ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])
									}else{
									var sel_cube = new THREE.Mesh( geometry,faceMaterial_obj[ch_to_tex(data_sarray[k][j].split('|')[0].split('.')[0])][0]);
									// sel_cube.name = ch_to_tex(data_array[k][j].split('|')[0].split('.')[0])
									}
								}
        	...
        	if(k == 0 && j == 0){
				...
				camera.rotateOnWorldAxis(Axis, THREE.Math.degToRad(-8));
		...
		if(mouseovered_group !== this.event.object.group){
			...
			if(mouseovered_group>-1){
				for (var i = group_array[mouseovered_group][1].length - 1; i >= 0; i--) {
					group_array[mouseovered_group][1][i].material = hover_mataerial_array[i]
				}
			}
			mouseovered_group = this.event.object.group
			hover_mataerial_array = []
			for (var i = group_array[mouseovered_group][1].length - 1; i >= 0; i--) {
				hover_mataerial_array.push( group_array[mouseovered_group][1][i].material)
				group_array[mouseovered_group][1][i].material = faceMaterial_obj[ch_to_tex(group_array[mouseovered_group][1][i].name)][2]
			}
			console.log(hover_mataerial_array)
...
// scene.background = new THREE.Color( 0xff0000 );
			...
			function pivotsky_move(){
				...
				skymove_a = skymove_a + degtorad(2.5)
					for (var i = pivot_wrapper_sky.length - 1; i >= 0; i--) {
						pivot_wrapper_sky[i].rotation.z = planevalue+skymove_a
					}
				// }else{
				skymove_b++
				// 	for (var i = pivot_wrapper_sky.length - 1; i >= 0; i--) {
		...
		function animate() {
			if(start){
				pivotsky_move()
				console.log(pivot_single_sky)
				console.log(pivot_wrapper_sky)
				start= false
			...
			for (var i = pivot_wrapper.length - 1; i >= 0; i--) {
				...
				if(i < 3){pivot_wrapper_sky[i].rotation.z = value+skymove_a}
				...
				if(i < 3){ pivot_wrapper_sky[i].position.z = result_sky }
				if(i < 3){ pivot_wrapper_sky[i].position.y= -0.5*getheight( Math.abs(result_sky),unit_obj*(radius-1)) }
