...
//  window.location = "http://orgd-list.org/textview";
// window.location.href = "http://orgd-list.org/textview";
// window.location.assign("http://orgd-list.org/textview");
// window.location.replace("http://orgd-list.org/textview");

  var first_post_height 
...
if(isMobile){
  if(window.location.href === 'http://orgd.org/2018/9/'){
        jQuery("#content").css("cssText", "overflow:visible !important;")
...
////////////////////////////////////////////////////////////////////////////////////
  ...
  if(hrefpostnumber === 'intro'){
      window.location.replace("http://orgd.org/2018/9/");
  }

    //   if(window.innerWidth/window.innerHeight< 86/100){

    // jQuery('article').find('a').each(function(){
    //     if(jQuery(this).outerHeight()>jQuery(this).outerWidth()){
    //     jQuery(this).css("cssText", "width:90% !important;")
    //     jQuery(this).css("marginRight", '5%')}
    //   })
    //   }
    if(hrefpostnumber === '9'){
      ...
      var maxcounter = Math.floor(jQuery('.entry-content').outerHeight()/(0.7*window.innerHeight))
      console.log(jQuery("#post-9").find('.entry-content').outerHeight())
      console.log(window.innerHeight*0.7)
      console.log(jQuery('.entry-content').outerHeight())
      console.log(maxcounter)
jQuery("#fromstart").click(function(){
        jQuery('.entry-content').animate({top : 0}, 600);
        jQuery("#arrow_bottom_wrap").show();
      counter = 0
})
      ...
      jQuery("#arrow_bottom").click(function () {
        console.log(hrefpostnumber)
      counter++
      jQuery('.entry-content').animate({top : -counter*0.7*h}, 600);
...
console.log(hrefpostnumber)
    if(!(isNaN(hrefpostnumber)||hrefpostnumber == 2018)){
      if(hrefpostnumber === '9'){}else{
      window.location.replace("http://orgd.org/2018/");
    }
    }
////////////////////////////////////////////////////////////////////////////////////
...
if(isMobile){
  if(window.location.href === 'http://orgd.org/2018/9/'){
        jQuery("content").css('overflow', 'visible');}else{
      ...
      var hashSplit = hashLocation.split("#!/");
      var hashName = hashSplit[1];
      if(hrefpostnumber == '2018'){
      history.pushState('',"",'http://orgd.org/2018/')
      ...
      if(window.innerWidth/window.innerHeight>= 86/100){
      jQuery('header').find('p').css("cssText", "width:100% !important");}
      jQuery("article").find("img").css({'width':'100%', 'marginLeft':'0%', 'left':'0'});
      jQuery('#posts').css('marginTop', posttop);
  ...
  jQuery('.content-area').css('height',first_post_height) 
  jQuery('#content').css('height',first_post_height)}
...
{
    jQuery("#cat_orgd").click(function () {
    var id_post = 11;
    var appendhere = jQuery('#post-11').find('.entry-content')
    ...
    $.ajax({
      ...
      if(window.innerWidth/window.innerHeight< 86/100){
      post_height = jQuery('#posts').innerHeight() + 0.072*window.innerHeight
      ...
      jQuery('#post-11').show();
      jQuery('#post-11').css('width', '100%');
      jQuery('#post-11').find('a').css('width', '100%');
      jQuery('#post-11').find('header').css('width', '100%');
      jQuery('#post-11').css('width', jQuery('#index').width());
      jQuery('#post-11').find("img").css({'width':'100%'});

      if(window.innerWidth/window.innerHeight< 86/100){
      jQuery("#index").css("transform", "scaleY(0.1)");
  post_height = jQuery('#posts').innerHeight() + 0.072*window.innerHeight
      console.log(post_height )
      console.log(jQuery('#posts').innerHeight())
      console.log(0.072*window.innerHeight)
  jQuery('.content-area').css('height',post_height) 
  jQuery('#content').css('height',post_height) }
    ...
    history.pushState({urlPath:'http://orgd.org/2018/'+idname+'/'},"",'http://orgd.org/2018/'+idname+'/')
    var id_post = idname;
    ...
    $.ajax({
      ...
      if(jQuery(this).attr('id')== 'post-14' ||
        jQuery(this).attr('id')== 'post-99' ||
        jQuery(this).attr('id')== 'post-126' ||
        jQuery(this).attr('id')== 'post-131' ||
        jQuery(this).attr('id')== 'post-120'){
...
var recentleft2
   ...
   return false;
    ...
    if(!(isNaN(hrefpostnumber_recent)||hrefpostnumber_recent == 2018)){/*so it's number*/
      // alert("nd")
      jQuery(".back").show();
      jQuery("article").hide();
    ...
    idname = jQuery(this).attr('id').split("-")[1];
      var id_post = hrefpostnumber_recent;
      var appendhere = jQuery("#"+hrefpost_recent).find('.entry-content')
    history.pushState({urlPath:'http://orgd.org/2018/'+hrefpostnumber_recent+'/'},"",'http://orgd.org/2018/'+hrefpostnumber_recent+'/')
    $.ajax({
        type: 'POST',
        url: ajax_object.ajax_url,
        data: {
        ...
        history.pushState({urlPath:'http://orgd.org/2018/'},"",'http://orgd.org/2018/')
          // alert("n")
      history.pushState('',"",'http://orgd.org/2018/')
...
if(window.innerWidth/window.innerHeight < 86/100){
  ...
  first_post_height = jQuery('#posts').innerHeight()+ jQuery('#index').innerHeight();
  jQuery('.content-area').css('height',first_post_height) 
  jQuery('#content').css('height',first_post_height) 

  jQuery('.content-area').css('overflow', 'hidden') 
  jQuery('#content').css('overflow', 'hidden') 
  if(window.location.href === 'http://orgd.org/2018/9/'){
        jQuery("#content").css("cssText", "overflow:visible !important;")
      }
