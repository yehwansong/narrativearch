...
class Thanks_Widget_Latest_Thanks extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'thanks_widget_latest_thanks', 'description' => __('the latest or the most thanked post titles with total thanks quant', 'thanks-you-counter-button'));
		parent::__construct('thanks-latest-thanks', __('Thanks Stat','thanks-you-counter-button'), $widget_ops);
...
function widget($args, $instance) {
  global $wpdb, $thanksCountersTable, $thanksNotCountablePostTypes;
	...
	$title = apply_filters('widget_title', empty($instance['title']) ? __('Thanks Stat', 'thanks-you-counter-button') : $instance['title']);
...
// number of rows to show
  ...
  if ($total || $content=='total_thanks') {
    ...
    $output .= apply_filters('thanks_stat_sidebar_total_quant', '<p class="thanks_total_quant"><span class="thanks_total_quant_label">'.__('Total quant of thanks: ','thanks-you-counter-button').'</span><span class="thanks_total_quant_value">'.$totalThanks.'</span></p>');
  }
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','thanks-you-counter-button'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>

    <label for="<?php echo $this->get_field_id('filter_by_category'); ?>" style="font-size:12px;"><?php _e('Filter by category:','thanks-you-counter-button'); ?></label>
...
?>      
		...
		<label for="<?php echo $this->get_field_id('number'); ?>" style="font-size:12px;"><?php _e('Number of posts to show:','thanks-you-counter-button'); ?></label>
...
?>
    ...
    <p><label for="<?php echo $this->get_field_id('content'); ?>"><?php _e('What posts to show:', 'thanks-you-counter-button'); ?></label>
      <select id="<?php echo $this->get_field_id('content'); ?>" name="<?php echo $this->get_field_name('content'); ?>" style="font-size: 0.9em;">
        <option value="latest_thanked" <?php echo thanks_optionSelected($content, 'latest_thanked'); ?>><?php _e('Latest thanked','thanks-you-counter-button');?></option>
        <option value="most_thanked" <?php echo thanks_optionSelected($content, 'most_thanked'); ?>><?php _e('Most thanked','thanks-you-counter-button');?></option>
        <option value="total_thanks" <?php echo thanks_optionSelected($content, 'total_thanks'); ?>><?php _e('Total quant of thanks','thanks-you-counter-button');?></option>
      </select>
    </p>
    <p>
      ...
      <label for="<?php echo $this->get_field_id('total'); ?>"><?php _e('Display total quant of thanks', 'thanks-you-counter-button'); ?></label>
    </p>
<?php
	} // end of form() method
...
add_action('widgets_init', 'thanks_widgets_init');
...
<div class="table">
  <table width="100%" cellpadding="0" cellspacing="0" class="widefat fixed">
    ...
    if ($showTotal==null || $showTotal==1) {
      $totalThanks = thanks_get_Total();
      $output .= '<div class="thanks_total_quant" style="text-align: right;margin-right:10px;">'.__('Total quant of thanks: ','thanks-you-counter-button').'<strong>'.$totalThanks.'</strong>'.'</div>';
    }
    $output .= '<div>';

    $showStatLink = get_option(THANKS_TD_STAT_LINK);
    if ($showStatLink==null || $showStatLink==1) {
      $output .= '<div style="float: right;margin-top:0px;font-size: 9px;"><a href="options-general.php?page=thankyou.php&paged=1">'.__('Check Full Statistics','thanks-you-counter-button').'</a></div>';
    }
    $showAuthorLink = get_option(THANKS_TD_AUTHOR_LINK);
    if ($showAuthorLink==null || $showAuthorLink==1) {
      $output .= '<div style="float: left;margin-top:0px;font-size: 9px;"><a target="_blank" href="http://www.shinephp.com/" title="'.__('Plugin author home page','thanks-you-counter-button').'">ShinePHP.com</a></div>';
    }
    $output .= '  </div>
                </div>';
    ...
    _e('No thanks yet', 'thanks-you-counter-button');
  }
...
<table>
	<tr>
		<td><label for="thanks_dashboard_rows_number"><?php _e('Posts number to show:', 'thanks-you-counter-button'); ?></label></td>
	...
	<tr>
		<td><label for="thanks_dashboard_content"><?php _e('What posts to show:', 'thanks-you-counter-button'); ?></label></td>
		<td colspan="2">
...
<label for="thanks_dashboard_total_show">
  ...
  <?php _e('Display total quant of thanks', 'thanks-you-counter-button'); ?>
</label><br/>
<label for="thanks_dashboard_statistics_link_show">
  ...
  <?php _e('Display Full Statistics link', 'thanks-you-counter-button'); ?>
</label><br/>
<label for="thanks_dashboard_author_link_show">
  ...
  <?php _e('Display plugin author link', 'thanks-you-counter-button'); ?>
</label>
...
function add_thanks_dashboard_widget() {
  $content = thanks_get_dashboard_content_kind();
  if ($content=='latest_thanked') {
    $widget_title = __('Latest Thanks', 'thanks-you-counter-button');
  } else {
    $widget_title = __('Most Thanked', 'thanks-you-counter-button');
  }
