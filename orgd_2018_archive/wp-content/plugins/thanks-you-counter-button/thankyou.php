...
Text Domain: thanks-you-counter-button
Domain Path: /lang
*/

/*
Copyright 2009-2016  Vladimir Garagulya  (email: vladimir [at] shinephp.com)
...
if (false) {
  __('Every time a new visitor clicks the "Thank you" button, one point is added to the total "thanks" counter for this post.', 'thanks-you-counter-button');
}

require_once(THANKS_PLUGIN_DIR .'/includes/thankyou_lib.php');

add_action('plugins_loaded', 'thanks_load_plugin_textdomain');

function thanks_load_plugin_textdomain() {
    
    load_plugin_textdomain('thanks-you-counter-button', FALSE, basename(dirname(__FILE__) ) . '/lang/' );
    
}
// end of thanks_load_plugin_textdomain()
...
?>
    ...
    <h2><?php _e('Thank You Counter Button Plugin', 'thanks-you-counter-button'); ?></h2>  
...
?>
    ...
    <div class="wrap">
    <div class="icon32" id="icon-options-general"><br/></div>
    <h2><?php _e('Thank You Counter Button Statistics', 'thanks-you-counter-button'); ?></h2>  
...
function thanks_buildButtonCode($thanksCaption = "", $siteGlobal = false) {
  ...
  if ($ipFound) {
    ...
    $buttonTitle = __('You left &ldquo;Thanks&rdquo; already for this post', 'thanks-you-counter-button');
  } else {
    $onButtonClick = 'thankYouButtonClick('.$post_id.', \''.thanks_esc_js(__('You left &ldquo;Thanks&rdquo; already for this post', 'thanks-you-counter-button')).'\')';
    $buttonTitle = __('Click to leave &ldquo;Thanks&rdquo; for this post', 'thanks-you-counter-button');
  }
  ...
  $button = '<div class="thanks_button_div" 
               ...
               ((is_user_logged_in() && current_user_can('manage_options') && (get_option('thanks_display_settings_shortcuts') == '1'))
               		?'<div class="thanks_settings_shortcuts">'.
               			'<a href="'.THANKS_WP_ADMIN_URL.'/options-general.php?page=thankyou.php" title="'.__('Settings','thanks-you-counter-button').'"><img class="thanks_shortcuts" height="8" width="8" alt="thank_you_settings" src="'.THANKS_PLUGIN_URL.'/images/settings.png" /></a>'.
               			'<a href="'.THANKS_WP_ADMIN_URL.'/tools.php?page=thankyou.php&amp;post_id='.$post_id.'&amp;paged=1#statistics" title="'.esc_attr(sprintf(__('View statistics details for "%s"', 'thanks-you-counter-button'), $post->post_title)).'"><img class="thanks_shortcuts" height="8" width="8" alt="thank_you_settings" src="'.THANKS_PLUGIN_URL.'/images/stats.png" /></a>'.
               			'<a title="'.__('Hide these shortcuts', 'thanks-you-counter-button').'" href="javascript:if(confirm(\''.thanks_esc_js(__('Do you really want to hide these shortcuts?', 'thanks-you-counter-button')).'\')) thankYouButtonRemoveSettingsShortcuts();"><img class="thanks_shortcuts" height="8" width="8" alt="thank_you_settings" src="'.THANKS_PLUGIN_URL.'/images/disable.png" /></a>'.
               			'</div>'
               		:'').
...
function thanks_install() {
    ...
    add_option('thanks_caption', __('Thank You', 'thanks-you-counter-button'));
...
function thanks_plugin_action_links($links, $file) {
    if ($file == plugin_basename(dirname(__FILE__).'/thankyou.php')){
        $settings_link = "<a href='options-general.php?page=thankyou.php'>".__('Settings','thanks-you-counter-button')."</a>";
...
function thanks_plugin_row_meta($links, $file) {
    if ($file == plugin_basename(dirname(__FILE__) . '/thankyou.php')) {
        $links[] = '<a target="_blank" href="http://www.shinephp.com/thank-you-counter-button-wordpress-plugin/2/#changelog">' . __('Changelog', 'thanks-you-counter-button') . '</a>';
        $links[] = '<a target="_blank" href="http://www.shinephp.com/thank-you-counter-button-wordpress-plugin/2/#filterhooks">' . __('Additional Documentation', 'thanks-you-counter-button') . '</a>';
    }
    return $links;
}
